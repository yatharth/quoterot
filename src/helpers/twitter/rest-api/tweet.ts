// Functions for fetching and dealing with single tweet objects.
...
// TODO: Consider pulling media fields, poll fields, and everything. Doesn’t count against any Twitter API quotas.
//  It would just use a bit more storage in DynamoDB or CloudWatch logs if we logged them there.

// For documentation on these fields, see:
//  https://developer.twitter.com/en/docs/twitter-api/tweets/lookup/api-reference/get-tweets

// export const allTweetParamsToFetch: Partial<Tweetv2FieldsParams> = {
...
export const tweetParamsToFetch: Partial<Tweetv2FieldsParams> = {
...
export async function fetchTweet(tweetId: string) {
    const result = await client.v2.singleTweet(tweetId, tweetParamsToFetch)
...
export function parseCanonicalTweetUrl(tweetUrl: string) {
...
export function getCanonicalUrl(tweet: TweetV2, includes: ApiV2Includes): string {
    const username = getAuthor(tweet, includes).username
    const url = `https://twitter.com/${username}/status/${tweet.id}`
    assert(canonicalTweetUrlPattern.test(url))
    return url
}

export async function fetchCanonicalUrl(tweet: TweetV2): Promise<string> {
    if (!tweet.author_id) throw "No author_id field in tweet."
    const author = await getUser(tweet.author_id)
