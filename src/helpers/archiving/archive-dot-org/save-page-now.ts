...
// TODO: Consider turning this into an npm package, under the name save-page-now,
//  since all the other packages seem to be broken.
...
export default class SavePageNow {
    ...
    async requestSave(url: string): Promise<SaveRequestResult> {
        ...
        const saveRequestOptions = {
            ...
            skip_first_archive: 1,  // Speeds up the archive by not checking if this is the first archive of the url.
            // capture_all: 1,  // Whether to capture page if it has an error HTTP response code.
        }

        // TODO: Seems like some complicated code. Could runtypes help with this?

        const data = await this.client.post('', {form: {url, ...saveRequestOptions}, headers: {"user-agent": "Googlebot"}}).json() as SaveRequest
        ...
        if ('status' in data && data.status == 'error') {
            ...
            console.error(`Unexpected response from SPN API for url ${url}`)
            console.error(jsonStringifyPretty(data))
            throw `Unexpected response from SPN API for url ${url}`
        }
